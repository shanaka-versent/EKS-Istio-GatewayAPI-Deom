...
# System Node Group
# When user node pool is enabled, system nodes get a taint so only system pods run here
# System pods: kube-system, argocd, istio-system, istio-ingress, aws-lb-controller
resource "aws_eks_node_group" "system" {
  ...
  # Taint system nodes when user node pool is enabled
  # This ensures application workloads go to user nodes
  dynamic "taint" {
    for_each = var.enable_user_node_pool && var.system_node_taint ? [1] : []
    content {
