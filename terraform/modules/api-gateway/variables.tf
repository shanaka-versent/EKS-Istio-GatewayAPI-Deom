...
variable "enable_cloudfront_protection" {
  description = "Enable CloudFront origin verification via Lambda Authorizer"
  type        = bool
  default     = false
}

variable "cloudfront_secret_header" {
  description = "Secret header value that CloudFront adds to requests (must match CloudFront origin config)"
  type        = string
