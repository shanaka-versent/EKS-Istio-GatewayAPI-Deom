# MTKC POC EKS - CloudFront Distribution with WAF
...
# - API Gateway origin for API traffic
...
resource "aws_cloudfront_cache_policy" "no_cache" {
  ...
  comment     = "No caching for dynamic content (API, web apps)"
  default_ttl = 0
  max_ttl     = 0
  min_ttl     = 0
...
resource "aws_cloudfront_origin_request_policy" "alb" {
  ...
  comment = "Origin request policy for ALB backend"
...
resource "aws_cloudfront_distribution" "main" {
  ...
  dynamic "origin" {
    ...
    content {
      ...
      custom_header {
        ...
        value = var.origin_verification_header
      }
    }
  }

  # API Gateway Origin
  # Protected with custom header that API Gateway validates via Lambda Authorizer
  dynamic "origin" {
    ...
    content {
      ...
      # API Gateway should validate this via Lambda Authorizer
      custom_header {
  ...
  # Default behavior (ALB or S3)
  default_cache_behavior {
    allowed_methods  = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = var.alb_dns_name != "" ? "ALB-web" : "S3-static-assets"

    cache_policy_id            = aws_cloudfront_cache_policy.no_cache.id
    origin_request_policy_id   = var.alb_dns_name != "" ? aws_cloudfront_origin_request_policy.alb.id : null
